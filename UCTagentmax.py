...
class ChoiceNode:
    ...
    def __init__(self, state: Game, next_tile: Tile, incoming_action: Optional[Action], parent: ChanceNode):
        ...
        self.visit_count = 0
        self.utilities = [0 for p in range(state.player_count)]
...
def UCT(node: ChoiceNode, parent_visit_count: int,  exploration_constant: float, current_player: int):
    if node.visit_count == 0:
        return "infinity"
    value = (node.utilities[current_player] / node.visit_count)
    exploration = exploration_constant * math.sqrt((2*math.log(parent_visit_count))/node.visit_count)
    return value + exploration


class UCTAgentMax(BaseAgent):
    ...
    def best_child(self, node: ChoiceNode, exploration_constant: float, current_player: int):
        children = node.children
        argmax = children[0]
        valmax = UCT(children[0], node.visit_count, exploration_constant, current_player)
        for child in children:
            uct_val = UCT(child, node.visit_count, exploration_constant, current_player)
            if (uct_val > valmax):
                argmax = child
                valmax = uct_val
        return argmax
    ...
    def tree_policy(self, current_node: Union[ChoiceNode, ChanceNode]):
        ...
        if type(current_node) == ChoiceNode:
            ...
            if current_node.has_expandable_actions():
                ...
                return self.tree_policy(self.best_child(current_node, self.exploration_constant, current_node.state.current_player))
        else:
    ...
    def default_policy(self, state: Game): 
        ...
        for player in range(len(scores)):
            other_scores = list(scores)
            own_score = other_scores.pop(player)
            best_opponent_score = max(other_scores)
            utilites.append(own_score - best_opponent_score)
        return utilites

    def backup(self, current_node: ChanceNode, payoff: list):
        # update reward and vist count for nodes on exploration path
        while(current_node):
            current_node.visit_count += 1
            current_node.utilities = [x+y for (x,y) in zip(current_node.utilities, payoff)]
    ...
    def uct_search(self, start_state: Game, next_tile: Tile, time_per_turn: int):
        ...
        while (time.time() - start) < time_per_turn:
            ...
            utilities = self.default_policy(simulate)
            # backup reward
            self.backup(node, utilities)
            #print("-----------------------------------------------------------------")
            #root.print_node() 
            i += 1         
    ...
    def make_move(self, next_tile: Tile):
        ...
        #print("Choose")
        best_action = self.best_child(root, 0, self.player_num).incoming_action
