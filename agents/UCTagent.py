...
class ChanceNode:
    ...
    def __init__(self, current_deck: list[Tile]):
        # available tiles to take from current state
...
class UCTAgent(BaseAgent):
    ...
    def expand(self, root: ChoiceNode):
        ...
        new_child = ChoiceNode()

    def tree_policy(self, root: ChoiceNode):
        current_node = root
    ...
    def uct_search(self, start_state: Game, incoming_action: Action, next_tile: Tile, iterations: int):
        # create root node
        root = ChoiceNode(copy.deepcopy(start_state), next_tile, incoming_action)
